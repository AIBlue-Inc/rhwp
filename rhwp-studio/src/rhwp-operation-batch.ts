/** Canonical HTATIS operation adapter. Raw WASM JSON contracts mirror worker.cjs.
 * Keep this module independent of the editor DOM so fixtures can exercise the bridge.
 */
import type { HwpDocument } from '../../pkg/rhwp';
...
export function insertPictureAtMatch(doc: OperationDocument, operation: StampOperation, imageDigest?: string) {
  ...
  // CommonObjAttr offsets are unsigned; a stamp hanging off the top/left paper edge is clamped
  // onto the paper (same policy as the HTATIS Node worker) and reported via clamped_to_paper.
  ...
  // object_ops.rs::insert_picture_in_paragraph_native maps that index through
