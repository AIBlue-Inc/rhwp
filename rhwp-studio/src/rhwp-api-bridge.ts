...
import { createOperationBridge, type OperationDocument } from './rhwp-operation-batch';
...
function rawDoc(): OperationDocument | null {
  ...
  return (d as OperationDocument | undefined) ?? null;
}
...
// flat operation contract is the source of truth. The adapter uses WasmBridge
// for existing operations and the raw document for the worker-compatible APIs.
// ─────────────────────────────────────────────────────────────
// main.ts imports this module before its exported wasm instance is initialized.
// Resolve dependencies only on the first request, after the import cycle completes.
let operationBridge: ReturnType<typeof createOperationBridge> | undefined;
function getOperationBridge() {
  return operationBridge ??= createOperationBridge({
    wasm, rawDoc, flushPendingNow,
  ...
  try {
    switch (method) {
      ...
      case 'applyOperationBatch': {
        reply(await getOperationBridge().applyOperationBatch(p.operations));
        break;
      }
      case 'readTargets': {
        reply(getOperationBridge().readTargets(p.targets));
        break;
      }
...
// SHA-256 uses Web Crypto; serialize requests so read-back/load/restore cannot
// overtake a batch while its image hashes are being prepared.
let requestQueue: Promise<void> = Promise.resolve();
window.addEventListener('message', (event: MessageEvent) => {
  if (!isOriginAllowed(event.origin) || event.data?.type !== 'rhwp-request'
    || !METHODS.has(event.data?.method)) return;
  requestQueue = requestQueue.then(() => handleMessage(event)).catch(error => {
